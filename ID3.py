...
def determinaDivisores_novo(indVar, listaIndices):
    arq = None
    if indVar == 3:
        arq = open("C:/Users/nitro5/PyCharmProjects/Implementação AM/.venv/Tabela Ordenada Var 3.txt")
    elif indVar == 4:
        arq = open("C:/Users/nitro5/PyCharmProjects/Implementação AM/.venv/Tabela Ordenada Var 4.txt")
    elif indVar == 5:
        arq = open("C:/Users/nitro5/PyCharmProjects/Implementação AM/.venv/Tabela Ordenada Var 5.txt")
    ...
    while len(linha) != 0:
        ...
        tabela[i][len(tabela[i]) - 1] = tabela[i][len(tabela[i]) - 1][:-1]
        if i < tam_fila:
            fila.append(tabela[i][7])
        else:
            resultados = [0] * 4
            for item in fila:
                resultados[int(item)-1] = resultados[int(item)-1] + 1

            resultados2 = [0] * 4
            resultados2[0] = resultados[0] / 234
            resultados2[1] = resultados[1] / 832
            resultados2[2] = resultados[2] / 404
            resultados2[3] = resultados[3] / 30
            print(resultados2)

            max = -1.1
            sel = 0
            for j in range(0, 4):
                if resultados2[j] > max:
                    max = resultados2[j]
                    sel = j
            historico.append(sel)
            fila.pop(0)
            fila.append(tabela[i][7])
        i += 1
        linha = arq.readline()
    ...
    print(historico)
    resultados_historico = [0] * 4

    for item in historico:
        resultados_historico[int(item)] = resultados_historico[int(item)] + 1
    ...
    for i in range(0, 4):
        cont = 0
        flag = 0
        for j in range(0, len(historico)):
            if historico[j] == i:
                cont += 1
            if cont > 0.33 * resultados_historico[i] and flag == 0:
                sel.append(j + tam_fila)
                flag = 1
            elif cont > 0.66 * resultados_historico[i]:
                sel.append(j + tam_fila)
                break

    divisores = []
    for item in sel:
        divisores.append(abs(float(tabela[item][3])))
